 ...
 */
class StreamLogger extends \Mustache\Logger\AbstractLogger
{
     ...
     *
     * @throws \Mustache\Exception\LogicException   If neither a stream resource nor url is present.
     * @throws \Mustache\Exception\RuntimeException If the stream url cannot be opened.
     *
    ...
    protected function writeLog($level, $message, array $context = array())
    {
        if (!is_resource($this->stream)) {
            if (!isset($this->url)) {
                throw new \Mustache\Exception\LogicException('Missing stream url, the stream can not be opened. This may be caused by a premature call to close().');
            }

            $this->stream = fopen($this->url, 'a');
            if (!is_resource($this->stream)) {
                // @codeCoverageIgnoreStart
                throw new \Mustache\Exception\RuntimeException(sprintf('The stream or file "%s" could not be opened.', $this->url));
